...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 550
			.LabelWidths = "100px"
			.Title = "traver mx moborig [profle name] origdn [MDN] cell [CellNo] [Term#] b"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Profile Name: sprint3, Cell No- 3039x", "terminate=newline")
			.AddItem Array("type=text", "name=s_strCascade", "label=Profile Name:", "accesskey=p", "value=" & s_strCascade, "size=15", "terminate=newline")
			.AddItem Array("type=text", "name=s_strPort", "label=MDN:", "accesskey=p", "value=" & s_strPort, "size=15", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=Cell No#:", "accesskey=s", "value=" & s_strSvlan, "size=15", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=Term#:", "accesskey=s", "value=" & s_strExtra, "size=15", "terminate=newline")
...
'			s_strSvlan = .Responses.Item("s_strSvlan")           
		...
		If (s_objUtils.TestLengthMinMax(s_strCascade, "Cascade ID", 0, 9)) Then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "traver mx moborig " & s_strCascade & " origdn " & s_strPort & " cell " & s_strSvlan & " " & s_strExtra & " b"
