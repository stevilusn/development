...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "150x"
			.Title = "FTW1 VSE: ssh [OIM Username]@[113.245.10.116]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter OIM Username:", "terminate=newline")
'			.AddItem Array("type=paragraph", "name=label1", "value= Example: al217441, IP Address 192.168.1.2", "terminate=newline")
			.AddItem Array("type=text", "name=s_strPort", "label=Username:", "accesskey=p", "value=" & s_strPort, "size=25", "terminate=newline")
'			.AddItem Array("type=text", "name=s_strSvlan", "label=IP address:", "accesskey=s", "value=" & s_strSvlan, "size=25", "terminate=newline")
			.Show
		...
		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 1, 10)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "ssh " & s_strPort & "@" & "113.245.10.116"
