...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "75px"
			.Title = "show config slot1 lan 1 [port] interface-module ddi"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Port 16", "terminate=newline")
...
'			s_strSvlan = .Responses.Item("s_strSvlan")           
		...
		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 2, 6)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show config slot1 lan 1 " & s_strPort & " interface-module ddi"
