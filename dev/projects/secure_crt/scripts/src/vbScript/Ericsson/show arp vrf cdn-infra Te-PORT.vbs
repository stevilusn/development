...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "75px"
			.Title = "show arp vrf cdn-infra | i Te-[port] "
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Port 1/2/3.1521", "terminate=newline")
...
'			s_strSvlan = .Responses.Item("s_strSvlan")           
		...
		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 1, 20)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show arp vrf cdn-infra | i Te"& s_strPort 
