...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "120px"
			.Title = "show arp-cache | inc [port]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Port 14/2", "terminate=newline")
...
'			.AddItem Array("type=text", "name=s_strPort", "label=Enter Port:", "accesskey=p", "value=" & s_strPort, "size=10", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=Enter Port:", "accesskey=s", "value=" & s_strSvlan, "size=10", "terminate=newline")
			.Show
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show arp-cache | inc " & s_strSvlan
