...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "75px"
			.Title = "show circuit counters [port] | include [s-vlan]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Port 14/2, S-VLan 3505", "terminate=newline")
		...
		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 3, 6) _
				And s_objUtils.TestLengthMinMax(s_strSvlan, "S-Vlan", 2, 6)) then
			blnTryAgain = False 
		End if
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show circuit counters " & s_strPort & " | include " & s_strSvlan
