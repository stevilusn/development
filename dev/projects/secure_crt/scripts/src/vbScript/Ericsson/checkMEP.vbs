...
Sub Main
	...
	blnRet = s_objUtils.SendAndWaitFor("", ("#" or ">"), s_intTimeout) 
...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "100px"
			.Title = "show bfd session | include [CSR ip address]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: CSR IP Address: 113.9.58.226", "terminate=newline")
...
'			.AddItem Array("type=text", "name=s_strPort", "label=Enter Port:", "accesskey=p", "value=" & s_strPort, "size=10", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=CSR IP Address:", "accesskey=s", "value=" & s_strSvlan, "size=10", "terminate=newline")
			.Show
...
'			s_strCascade = .Responses.Item("s_strCascade")   
		...
		If (s_objUtils.TestLengthMinMax(s_strSvlan, "CSR IP Addrss", 7, 15)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show config cfm domain md5 ma ma5 mep 33" & chr(13) & _
    "show config cfm domain md5 ma ma7 mep 34 " & chr(13) & _
    "show config cfm domain md6 ma ma6 mep 11 " & chr(13) & _
    "show config cfm domain md6 ma ma8 mep 12 " & chr(13)
