...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "75px"
			.Title = "show bfd session | i [IP Address] "
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: IP Address 113.254.47.1", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=Enter IP:", "accesskey=p", "value=" & s_strSvlan, "size=10", "terminate=newline")
...
'			s_strSvlan = .Responses.Item("s_strSvlan")           
		...
		If (s_objUtils.TestLengthMinMax(s_strSvlan, "Port Number", 3, 12)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show bfd session | i "& s_strSvlan 
