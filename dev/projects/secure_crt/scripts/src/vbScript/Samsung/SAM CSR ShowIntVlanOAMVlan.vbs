...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "100px"
			.Title = "show int vlan [vlan]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter a VLan, example a EOAM or BH Magmnt VLan:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: VLan 1234", "terminate=newline")
...
'			.AddItem Array("type=text", "name=s_strPort", "label=Enter Port:", "accesskey=p", "value=" & s_strPort, "size=10", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=Enter OAM VLan:", "accesskey=s", "value=" & s_strSvlan, "size=10", "terminate=newline")
			.Show
...
'			s_strCascade = .Responses.Item("s_strCascade")   
		...
		If (s_objUtils.TestLengthMinMax(s_strSvlan, "EOAM VLan", 2, 9)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show int vlan " & s_strSvlan
