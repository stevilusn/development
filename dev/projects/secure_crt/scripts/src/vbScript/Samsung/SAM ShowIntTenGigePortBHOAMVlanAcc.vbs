...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "125px"
			.Title = "show Int TenGige [port.VLan] acc"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter port and Vlan OAM VLan, BH Mgmt VLan):", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Port 0/6/0/3, VLan 1738", "terminate=newline")
			.AddItem Array("type=text", "name=s_strPort", "label=Enter Port:", "accesskey=p", "value=" & s_strPort, "size=10", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=Enter VLan:", "accesskey=s", "value=" & s_strSvlan, "size=10", "terminate=newline")
			.Show
		...
		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 3, 12) _
				And s_objUtils.TestLengthMinMax(s_strSvlan, "S-Vlan", 2, 9)) then
			blnTryAgain = False 
		End if
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show int tengige " & s_strPort & "." & s_strSvlan & " acc"
