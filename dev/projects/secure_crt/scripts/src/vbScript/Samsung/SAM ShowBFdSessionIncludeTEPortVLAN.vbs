...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "125px"
			.Title = "show bfd session | inc Te[port].[c-vlan]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Port 0/6/0/3,    C-VLan 1738", "terminate=newline")
			.AddItem Array("type=text", "name=s_strPort", "label=Enter Port:", "accesskey=p", "value=" & s_strPort, "size=10", "terminate=newline")
			.AddItem Array("type=text", "name=s_strSvlan", "label=Enter C-VLan:", "accesskey=s", "value=" & s_strSvlan, "size=10", "terminate=newline")
			.Show
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "Show bfd session | inc Te" & s_strPort & "." & s_strSvlan
