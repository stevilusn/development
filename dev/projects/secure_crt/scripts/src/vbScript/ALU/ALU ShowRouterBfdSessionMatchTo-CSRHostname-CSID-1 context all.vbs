...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "175px"
			.Title = "show router bfd session | match [to-CSRHOSTNAME-CSID-1] context all"
			.AddItem Array("type=paragraph", "name=label1", "value= Enter CSRHostname-CellSiteID-X from the LTE Lookup Tool or NRM:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: CHRMNCGF-BHRS-1", "terminate=newline")
			.AddItem Array("type=text", "name=s_strPort", "label=Enter CSRHostname-CSID-X:", "accesskey=p", "value=" & s_strPort, "size=15", "terminate=newline")
...
'			s_strSvlan = .Responses.Item("s_strSvlan")           
		...
		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 3, 20)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show router bfd session | match to-" & s_strPort & " context all"
