...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "75px"
			.Title = "show log log-ig [Log ID #]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter exact requested information:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: 99 or 100", "terminate=newline")
			.AddItem Array("type=text", "name=s_strPort", "label=Enter Log ID:", "accesskey=p", "value=" & s_strPort, "size=10", "terminate=newline")
...
'			s_strSvlan = .Responses.Item("s_strSvlan")           
		...
		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 1, 4)) then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show log log-id " & s_strPort
