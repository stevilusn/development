...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "100px"
			.Title = "show router 2 interface | match [cascade]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter Cascasde ID:", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: Cascade ID AT03GI203", "terminate=newline")
...
'			s_strSvlan = .Responses.Item("s_strSvlan")           
		...
		If (s_objUtils.TestLengthMinMax(s_strCascade, "Cascade ID", 9, 9)) Then
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show router interface | match " & s_strCascade
