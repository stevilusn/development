...
Function get_user_data()
	...
	Do 	
		With s_objDialogBox
			...
			.Width = 375
			.LabelWidths = "75px"
			.Title = "show service sdp-using [S-Vlan ID]"
			.AddItem Array("type=paragraph", "name=label1", "value= Please enter S-Vlan (EVC):", "terminate=newline")
			.AddItem Array("type=paragraph", "name=label1", "value= Example: 1234", "terminate=newline")
...
'		If (s_objUtils.TestLengthMinMax(s_strPort, "Port Number", 3, 15))
			blnTryAgain = False 
		End if
...
Sub issue_PORTSVLAN_command()
	s_objUtils.Send "show service sdp-using " & s_strPort
